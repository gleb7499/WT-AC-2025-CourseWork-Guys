...
### Настройка hosts файла (если не сделано ранее)

```powershell
Add-Content -Path "C:\Windows\System32\drivers\etc\hosts" -Value "`n127.0.0.1 app-dev.local" -Encoding UTF8

# http://app-dev.local теперь должен резолвиться на localhost
```
